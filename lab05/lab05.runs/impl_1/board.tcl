...
namespace eval ::optrace {
  variable script "D:/CS/lab/lab05/lab05.runs/impl_1/board.tcl"
...
OPTRACE "Write Bitstream: pre hook" START { }
  set src_rc [catch { 
    puts "source D:/CS/lab/lab05/def.tcl"
    source D:/CS/lab/lab05/def.tcl
  } _RESULT] 
  if {$src_rc} { 
    set tool_flow [get_property -quiet TOOL_FLOW [current_project -quiet]]
    if { $tool_flow eq {SDx} } { 
      send_gid_msg -id 2 -ssname VPL_TCL -severity ERROR $_RESULT
      send_gid_msg -id 3 -ssname VPL_TCL -severity ERROR "sourcing script D:/CS/lab/lab05/def.tcl failed"
    } else {
      send_msg_id runtcl-1 status "$_RESULT"
      send_msg_id runtcl-2 status "sourcing script D:/CS/lab/lab05/def.tcl failed"
    }
    return -code error
  }
